...
void fconfig_USB_FS_device(USB_GPIO_t dp, USB_GPIO_t dn, uint32_t RX_FIFO_size) {
	...
	/* GPIO config */
	fconfig_GPIO(dp_port, dp_pin.num, GPIO_alt_func, GPIO_no_pull, GPIO_push_pull, GPIO_very_high_speed, dp_pin.alt);
	fconfig_GPIO(dn_port, dn_pin.num, GPIO_alt_func, GPIO_no_pull, GPIO_push_pull, GPIO_very_high_speed, dn_pin.alt);
	...
	*PCGCCTL &= ~(
	...
	/* setup FIFO buffers */
	config_USB_RX_FIFO(usb, RX_FIFO_size);
	config_USB_TX_FIFO(usb, 0, 0x40);		// max setup packet size
}
